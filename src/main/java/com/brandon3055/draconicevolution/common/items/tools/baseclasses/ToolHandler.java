...
public class ToolHandler {
    ...
    public static void damageEntityBasedOnHealth(Entity entity, EntityPlayer player, float dmgMult) {
        ...
        if (entity instanceof EntityDragonPart) {
            List<EntityDragon> list = player.worldObj
                    .getEntitiesWithinAABB(EntityDragon.class, entity.boundingBox.expand(10, 10, 10));
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            for (d0 = player.posZ - entityLivingBase.posZ; d1 * d1 + d0 * d0
    ...
    public static void AOEAttack(EntityPlayer player, Entity entity, ItemStack stack, int range) {
        ...
        for (Object entityObject : list) {
            ...
            if (entityObject instanceof EntityLivingBase) {
                ...
                if (EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, stack) > 0) entityLivingBase
                        .setFire(EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, stack) * 15);
    ...
    public static MovingObjectPosition raytraceFromEntity(World world, Entity player, double range) { // todo move to
                                                                                                      // core and make
                                                                                                      // the range work
                                                                                                      // as expected
        float f = 1.0F;
